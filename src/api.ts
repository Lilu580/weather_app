...
export const getWeather = (city: string) => get(WEATHER_URL, `&q=${city}&days=14&aqi=no&alerts=yes`);
export const getCities = (query: string) => get(CITIES_URL, `&q=${query}`);
export const getHistoryWeather = (city: string, date: string) => get(HISTORY_URL, `&q=${city}&dt=${date}`);
export const getFutureWeather = (city: string, date: string) => get(FUTURE_URL, `&q=${city}&dt=${date}`);